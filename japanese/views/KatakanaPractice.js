...
export default {
  ...
  computed: {
    ...
    kanaStats() {
      return getStats('kana-katakana', this.KANA);
    },
    wordMemory() {
    ...
    wordStats() {
      ...
      return getStats('words-katakana', this.words.map(({ word }) => word));
    },
  ...
  methods: {
    ...
    buildKanaOptions(correctKana) {
      ...
      return shuffle([correctKana, ...wrongs]).map(k => ({ kana: k, romaji: katakana[k] }));
    },
    newTarget() {
    ...
    handleKana(candidate) {
      ...
      recordAnswer('kana-katakana', this.target, correct);
    ...
    onInput(e) {
      ...
      const raw = [...data].at(-1);
      const candidate = kanaMap[raw] || raw;
