...
const CARD_API = 'db.ygoprodeck.com/api/v7/cardinfo.php?name=';
...
createApp({
  ...
  methods: {
    ...
    getBGByType( card ) {
      if( !card ) return '-none';
      const { frameType } = card;
      const cardType = frameType.toLowerCase();
    ...
    selectCard( card ) {
      ...
      this.selectedCardAttribute = undefined;
      this.selectedCardPrices = undefined;
      this.showLevelOrRank = false;
      this.isLinkMonster = false;

      setTimeout(() => {
        this.selectedCard = card;
      }, 0);

      getAPI(CARD_API + card.name)
        .then( ({ data }) => {
          console.log(data);
          ...
          const _data = data[0];
          this.selectedCardData = data[0];
          this.selectedCardImage = _data.card_images[0].image_url;
          this.selectedCardDesc = _data.desc.replace(/\r\n/g, '<br>');
          this.selectedCardAttribute = _data.attribute?.toLowerCase()
            || _data.frameType;
